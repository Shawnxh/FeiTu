...
(function ($) {
  $.fn.swipeslider = function (options) {
    ...
    var settings = $.extend(defaultSettings, options); // Privates //
    ...
    */
    ...
    var pixelOffset = 0;
    var currentSlide = 0;
    var slideCount = 0; // Overall width of sliders.

    var slidesWidth = 0; // Flag for disbling swipe function while transition animation is playing.

    var allowSwipe = true;
    var transitionDuration = settings.transitionDuration;
    var swipe = settings.swipe;
    var autoPlayTimeout = settings.autoPlayTimeout; // ID of timeout function that waits for animation to end.
    ...
    */
    ...
    (function init() {
      ...
      slidesWidth = slider.width(); // console.log(slidesWidth);
      // Change slide width when window changes.

      $(window).resize(resizeSlider);

      if (settings.prevNextButtons) {} // insertPrevNextButtons();
    ...
    */
    ...
    function swipeStart(event) {
      ...
      disableAutoPlay(); // If it is mobile device redefine event to first touch point

      if (event.originalEvent.touches) event = event.originalEvent.touches[0]; // Check if slide started on slider 
    ...
    */
    ...
    function swiping(event) {
      var pointerData; // Get pointer data from event.
      ...
      var deltaSlide = pointerData.clientX - startClientX; // If sliding started first time and there was a distance.
      ...
      if (slidingState == 2) {
        ...
        var touchPixelRatio = 1; // Check for user doesn't slide out of boundaries

        if (currentSlide == 0 && pointerData.clientX > startClientX || currentSlide == slideCount - 1 && pointerData.clientX < startClientX) {
        ...
        enableTransition(false); // Apply moving and remove animation class
    ...
    */
    ...
    function swipeEnd(event) {
      if (slidingState == 2) {
        // Reset sliding state.
        slidingState = 0; // Calculate which slide need to be in view.

        currentSlide = pixelOffset < startPixelOffset ? currentSlide + 1 : currentSlide - 1; // Make sure that unexisting slides weren't selected.

        currentSlide = Math.min(Math.max(currentSlide, 0), slideCount - 1); // Since in this example slide is full viewport width offset can be calculated according to it.
    ...
    */
    ...
    function translateX(distance) {
      slider // Prefixes are being set automatically.
    ...
    */
    ...
    function setTransitionDuration(duration) {
      slider //      .css('-webkit-transition-duration', duration + 'ms')
    ...
    */
    ...
    function setTimingFunction(functionDescription) {
      slider //      .css('-webkit-transition-timing-function', functionDescription)
    ...
    */
    ...
    function setTransitionProperty(property) {
      slider //      .css('-webkit-transition-property', property)
    ...
    */
    ...
    function insertBullets(count) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        var item = slideContainer.find('.sw-slide-' + i); // Workaround a problem when iterator i will have max value due to closure nature.
