...
var VRUtils = function VRUtils(vr) {
  this.vr = vr;
...
VRUtils.prototype.markIcon = function (img, position, name, title, w, h) {
  var w = w || 0.08;
  var h = h || 0.08;
  var textureLoader = new THREE.TextureLoader(); // 定义颜色贴图，渲染纹理图两面
  ...
  div.id = name;
  div.style = "width:fit-content;text-align:center;padding:4px 4px;background:rgba(0,0,0,.5);color:#fff;display:none;position:absolute;border-radius:6px; -webkit-user-select:none; -moz-user-select:none; -ms-user-select:none; user-select:none;font-size:0.75rem;";
  div.innerHTML = title;
...
VRUtils.prototype.markIconInViews = function () {
  ...
  for (var i = 0; i < this.markIconGroup.children.length; i++) {
    ...
    if (pos.x >= -1 && pos.x <= 1 && pos.y >= -1 && pos.y <= 1 && pos.z >= -1 && pos.z <= 1) {
      var screenPos = AVR.objectPosToScreenPos(this.markIconGroup.children[i], this.vr.container, this.vr.camera); // console.log(name);
...
VRUtils.prototype.bindRaycaster = function (event, callback) {
  var vector = AVR.screenPosTo3DCoordinate(event, this.vr.container, this.vr.camera); //在视点坐标系中形成射线,射线的起点向量是照相机， 射线的方向向量是照相机到点击的点，这个向量应该归一标准化。

  var raycaster = new THREE.Raycaster(this.vr.camera.position, vector.sub(this.vr.camera.position).normalize()); //射线和模型求交，选中一系列直线
