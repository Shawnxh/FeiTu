let $appid, $timestamp, $noncestr, $signature, $description, $title1;

// 微信分享调用接口(share的api暂时只能用http这个)
const protocol_domain_port = 'http://39.102.65.183:8080/open';
// 链接地址
let $linkUrl = "https://ftplayer.cdflytu.com/mxreality/laiyin/examples/main.html";
// 分享图片 => 线上地址
...
$title1 = "VR互动全景：德国莱茵集团";



//获取当前页面的url => 完整url 
let $linkUrlToken = window.location.href;

// encodeURIComponent(),请求后台接口需要用encodeURIComponent()
let encodeUrl = encodeURIComponent($linkUrlToken);
